 ...
 */
abstract class TreeNodeController extends Controller {

    protected $_nodeModelClass = null;
    ...
    protected static function checkValidRequest($isJsonResponse = true, $isInvalid = null) {
        ...
        if ($isInvalid) {
            throw new InvalidCallException(Yii::t('app.c2', 'This operation is not allowed.'));
        }
    }
     ...
     * @throws InvalidCallException
     */
    protected function checkSignature($action, $data = []) {
        if (Yii::$app instanceof Application) {
            return; // skip hash signature validation for console apps
        }
        $module = $this->module;
        $security = Yii::$app->security;
        $modelClass = $this->getNodeModelClass();
        $validate = function ($act, $oldHash, $newHashData) use ($module, $security) {
            ...
            $message = Yii::t(
                            'app.c2', '<h4>Operation Disallowed</h4><hr>Invalid request signature detected during tree data <b>{action}</b> action! Please refresh the page and retry.{params}', ['action' => $act, 'params' => $params]
            );
     ...
     */
    public function actionNodeSave() {
        ...
        if ($treeNodeModify) {
            $node = new $modelClass;
            $successMsg = Yii::t('app.c2', 'The node was successfully created.');
            $errorMsg = Yii::t('app.c2', 'Error while creating the node. Please try again later.');
        } else {
            $tag = explode("\\", $modelClass);
            $tag = array_pop($tag);
            $id = $post[$tag][$keyAttr];
            $node = $modelClass::findOne($id);
            $successMsg = Yii::t('app.c2', 'Saved the node details successfully.');
            $errorMsg = Yii::t('app.c2', 'Error while saving the node. Please try again later.');
        }
        ...
        if ($node->save()) {
            ...
            if (!empty($errors)) {
                ...
                foreach ($errors as $err) {
                    $errorMsg .= "<li>" . Yii::t('app.c2', "Node # {id} - '{name}': {error}", $err) . "</li>\n";
                }
                $errorMsg .= "</ul>";
            }
        } else {
     ...
     */
    public function actionNodeManage() {
        ...
        $callback = function () {
            ...
            $this->checkSignature('manage', $data);
            return $this->renderAjax($nodeView, ['model' => $node, 'params' => $params]);
        };
        return self::process(
                        $callback, Yii::t('app.c2', 'Error while viewing the node. Please try again later.'), null
        );
    }
     ...
     * Remove a tree node
     */
    public function actionNodeRemove() {
        static::checkValidRequest();
        $callback = function () {
            /**
             * @var Tree $modelClass
            ...
            $modelClass = $this->getNodeModelClass();
            $data = static::getPostData();
            $this->checkSignature('remove', $data);
            extract($data);
            $softDelete = false;
            $node = $modelClass::findOne($id);
        ...
        return self::process(
                        $callback, Yii::t('app.c2', 'Error removing the node. Please try again later.'), Yii::t('app.c2', 'The node was removed successfully.')
        );
    }
    ...
    public function actionNodeMove() {
        ...
        $errorMsg = $isMovable ? Yii::t('app.c2', 'Error while moving the node. Please try again later.') :
                Yii::t('app.c2', 'The selected node cannot be moved.');
        $callback = function () use ($dir, $nodeFrom, $nodeTo, $allowNewRoots, $treeMoveHash, $isMovable, $data, $errorMsg) {
            if (!empty($nodeFrom) && !empty($nodeTo)) {
                $this->checkSignature('move', $data);
                if (!$isMovable) {
                    return false;
                }
                if ($dir == 'u') {
                ...
                if (!$nodeFrom->save()) {
                    $errorMsg = implode(',', $nodeFrom->getFirstErrors());
        ...
        return self::process($callback, $errorMsg, Yii::t('app.c2', 'The node was moved successfully.'));
    }

    public function getNodeModelClass() {
        if (is_null($this->_nodeModelClass)) {
            throw new \yii\web\HttpException(404, "NodeModelClass is required!");
        }
        return $this->_nodeModelClass;
    }
