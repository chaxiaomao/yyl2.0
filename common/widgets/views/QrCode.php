...
$qrCodeImageAttribute = $this->context->qrCodeImageAttribute;

$qrCodeImageModel = $model->getOneAttachment($qrCodeImageAttribute);
...
<div class="file-input file-input-ajax-new">
    <div class="file-preview ">
        <div class=" file-drop-zone">
            <?php if ($model->{$qrCodeAttribute}): ?>
                <div class="file-preview-thumbnails">
                    <?php echo $qrCodeImageStr ?>
                </div>
            <?php else: ?>
                <div class="file-drop-zone-title"><?= Yii::t('app.c2', 'My QrCode') ?><br></div>
            <?php endif ?>
...
if ($readonly) {
    $js.= "jQuery(document).off('click', '#{$linkId}').on('click', '#{$linkId}', function(e, data){
                ...
                jQuery.pjax({
                    ...
                    container: '#{$pjaxId}',
                    data: {id:'{$model->id}', codeAttribute:'{$qrCodeAttribute}', imageAttribute:'{$qrCodeImageAttribute}', content:''},
                    push:false
                  });
            });";
} else {
    $js.= "jQuery(document).off('click', '#{$linkId}').on('click', '#{$linkId}', function(e, data){
                ...
                jQuery.pjax({
                    ...
                    url: '{$this->context->action}',
                    container: '#{$pjaxId}',
                    data: {id:'{$model->id}', codeAttribute:'{$qrCodeAttribute}', imageAttribute:'{$qrCodeImageAttribute}', content:jQuery('#" . Html::getInputId($model, $qrCodeAttribute) . "').val() },
                    push:false
                  });
            });";
}
