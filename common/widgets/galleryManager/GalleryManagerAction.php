...
//        return true;
    ...
    protected function actionMore($page, $pid) {
        $pageSize = 10;
        $images = array();
        if ($this->modelClass == EntityAttachment::className()) {
            foreach ($this->behavior->getAttachmentImagesMore(($page - 1) * $pageSize, $pageSize) as $image) {
                $images[] = array(
                    'id' => $image->id,
            ...
            $pageCount = $this->behavior->getAttachImagePageCount(($page - 1) * $pageSize, $pageSize, $pid);
            //Yii::info('$pageCount'.$pageCount);
            return json_encode(['images' => $images, 'page_count' => $pageCount]);
        } else {
            foreach ($this->behavior->getImagesMore(($page - 1) * $pageSize, $pageSize, $pid, ['type' => CmsBlockType::TYPE_ALBUM, 'block_id' => $pid]) as $image) {
                $images[] = array(
                    'id' => $image->id,
                    'position' => $image->position,
                    'label' => (string) $image->label,
            ...
            $pageCount = $this->behavior->getImagePageCount(($page - 1) * $pageSize, $pageSize, $pid, ['type' => CmsBlockType::TYPE_ALBUM, 'block_id' => $pid]);
            return json_encode(['images' => $images, 'page_count' => $pageCount]);
        }
    }
    ...
    protected function getFileName($imageId, $version = 'original') {
        if ($this->modelClass == EntityAttachment::className()) {
            $entity = EntityAttachment::findOne($imageId);
    ...
    protected function actionDelete($ids) {
        //Yii::info($ids);
        if ($this->modelClass == EntityAttachment::className()) {
            EntityAttachment::deleteAll(['in', 'id', $ids]);
        } else {
