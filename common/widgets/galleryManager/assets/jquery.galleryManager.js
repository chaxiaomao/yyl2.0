(function ($) {
  ...
  function galleryManager(el, options) {
    ...
    var photoTemplate = '<div class="photo">' + '<div class="image-preview" style="display: block">' +
        '<a class="preview" data-fancybox="gallery" href=""><img src=""/></a></div><div class="caption">';
    if (opts.hasName) {
      photoTemplate += '<h5></h5>';
    }
    if (opts.hasDesc) {
    ...
    function editPhotos(ids) {
      ...
      for (var i = 0; i < l; i++) {
        ...
        //     '</iframe>');
        if (opts.updateUrl){
          window.open(opts.updateUrl +"?id="+id);
        }else{
