...
class GalleryBehavior extends Behavior
{
     ...
     */
    public $tableName = '{{%cms_block_item}}';
     ...
     * @return GalleryImage[]
     */
    public function getImages($offset=2,$limit=2,$condition=[])
    {
        if ($this->_images === null) {
            ...
            foreach ($imagesData as $imageData) {
                if ($this->tableName === '{{%cms_block_item}}' ){
                    $images[] = new GalleryImage($this, $imageData);
                }else{
                    $images[] = new GalleryImage($this, $imageData,true);
                }            }
        }
     ...
     * @return GalleryImage[]
     */
    public function getImagesMore($offset=2,$pageSize=2,$pid,$condition=[])
    {
            $query = new \yii\db\Query();
        $imagesData =  $query
                ->from($this->tableName)
...
//            $imagesData = $query->all();
     ...
     * @return GalleryImage[]
     */
    public function getAttachmentImagesMore($offset=2,$pageSize='')
    {
        $query = new \yii\db\Query();
        $query = $query
    ...
    public function getImagePageCount($offset=5,$pageSize,$pid,$condition=[])
    {
            $query = new \yii\db\Query();
            $count = $query
                ->from($this->tableName)
                ->where($condition)
                ->orderBy(['possition' => 'asc'])->count();
        return ceil($count/$pageSize);
    }

    public function getAttachImagePageCount($offset=5,$pageSize,$pid,$condition=[])
    {
        $query = new \yii\db\Query();
        $count = $query
            ->from('{{%entity_attachment}}')
    ...
    protected function getFileName($imageId, $version = 'original')
    {
        ...
        //\Yii::info('路径' . $entity->logic_path . '/' .$entity->hash . '.' . $entity->extension);
    ...
    public function getUrl($imageId, $version = 'original')
    {
        ...
        //\Yii::info('imageId' . $imageId);
        if (!file_exists($path)) {
            return null;
        }
        return \Yii::$app->params['config']['upload']['accessUrl'] . '/' . $this->getFileName($imageId, $version);
    }

    public function getFilePath($imageId, $version = 'original')
    {
        return \Yii::getAlias('@appimage') . '/' . $this->getFileName($imageId, $version);
    }
...
//        $pk = $this->owner->getPrimaryKey();
        return \Yii::$app->request->get('pid');
        if (is_array($pk)) {
    ...
    public function updateImagesData($imagesData)
    {
        ...
        if ($this->_images !== null) {
            ...
            $rawImages = (new Query())
                ->select(['id', 'label', 'content', 'link'])
                ->from($this->tableName)
                ->where(['type' => $this->type, 'block_id' => $this->getGalleryId()])
