...
class FeUserModel extends \cza\base\models\ActiveRecord implements IdentityInterface
{
    ...
    {
        if ($this->type == FeUserType::TYPE_SYS_REGISTRATION) {
            return $this->getInitialPreview('avatar', \cza\base\models\statics\ImageSize::ORGINAL);
        } elseif($this->type == FeUserType::TYPE_WECHAT_REGISTRATION) {
            $initialPreview = [];
            $initialPreview[] = Html::img($this->unconfirmed_email, ['class' => 'file-preview-image', 'style' => 'width:300px;height:250px']);
